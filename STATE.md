...
7. Adopt DataShell + Table (task-28 follow-on) across the 6 data pages (Tasks/Contacts/Conversations/
   Dashboard/Home/Settings), replacing the loading/error/empty ternary + hand-rolled `<table>`/TH-TD
   class constants. Heads-up: Contacts' header gains `font-medium` on adoption (the `TH` primitive
   standardizes it) — intended, not a regression.
## IN FLIGHT

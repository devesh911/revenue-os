...
- (this PR) T5 channels guarded doorway (`packages/channels`) — `voice.place` (Vapi) + `wa.send` (Meta) build a `GuardedAction` (channel/contactId/tool identity), `await guard()` before any sender call, block with the `Verdict` on `!ok` (no send), only-then call the injected sender. Ports-injected guard/senders keep the harness dep type-only (`@harness/*` alias, erased at runtime) — no BOM/package.json change. typecheck+lint clean (160 files), `channels.test.ts` 7/7 (31 expect), env-free; wa.send vars-merge deferred to T7 — 2026-07-25

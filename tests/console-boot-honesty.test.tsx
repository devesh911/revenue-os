// The lazy getSupabase() singleton (#53) — RED spec (adapts the #49 boot-honesty suite).
//
...
// ./app/App behind the parseConsoleEnv gate (and needed a BootErrorScreen .catch). #53 removes
...
//   - The lazy-client checks on lib/supabase.ts and main.tsx — the #53 refactor — landed; they stay
//     as regression guards.
//   - The #49 behaviors must STAY GREEN as regression guards. The sign-in front-door PR
//     changed two of its OrgHomeView cases: EMPTY now reads "You're not in a workspace yet"
//     (RED until that lands), and ERROR passes the network ApiError the view now branches on.
//   - The old fixed-file import check was REPLACED by the sign-in front-door PR's rule-based check
//     over every file in apps/console/src: the old three-file list named auth.tsx, which that PR
//     retires.
...
describe("lazy getSupabase() singleton — console boot honesty", () => {
  ...
  // Source check — "lib/supabase.ts must perform NO createClient(...) at module-evaluation time …
  ...
  it("lib/supabase.ts constructs no client at module scope and exports getSupabase (source)", async () => {
  ...
  // Behavioral check (env-free, createClient mocked) — importing the module must construct
  ...
  it("importing lib/supabase.ts (env unset) builds nothing until getSupabase(), then memoizes one client", async () => {
    let built = 0;
  ...
  // Source check — main.tsx STATICALLY imports App (no dynamic import("./app/App")) now that
  ...
  it("main.tsx statically imports App and keeps the parseConsoleEnv → ConfigErrorScreen gate", async () => {
  ...
  // ── Preserved #49 behaviors — regression guards, must STAY GREEN on today's code ──

  // parseConsoleEnv reports each required var that is missing OR empty, by name.
  it("parseConsoleEnv reports each required var that is missing or empty, by name", async () => {
  ...
  // ConfigErrorScreen names each missing var and points to apps/console/.env.example.
  it("ConfigErrorScreen names each missing var and points to apps/console/.env.example", async () => {
  ...
  // With both vars present, parsing succeeds and carries the values (no error screen).
  it("parseConsoleEnv returns ok and carries the values when both vars are present", async () => {
    const { parseConsoleEnv } = await import("../apps/console/src/lib/env");
  ...
  // Updated for the sign-in landing states — OrgHomeView NETWORK error names the API base URL and
  // reads as unreachable, not empty. It now receives the error itself (the view tells "unreachable" from
  // other failures by it) plus the Retry / Sign out handlers.
  it("OrgHomeView NETWORK error names the API base URL and reads as unreachable, not empty", async () => {
  ...
  // Updated for the sign-in landing states — OrgHomeView EMPTY list reads as "not in a workspace
  // yet" (the old "No orgs yet — create one via the API" hint is gone: accounts are invite-only) —
  // never an outage.
  it("OrgHomeView EMPTY reads 'You're not in a workspace yet', not an outage", async () => {
    const { OrgHomeView } = await import("../apps/console/src/app/OrgHomeView");
    const html = renderToStaticMarkup(
      <OrgHomeView
        isLoading={false}
  ...
  // OrgHomeView LOADING is unchanged ("Loading orgs").
  it("OrgHomeView LOADING is unchanged ('Loading orgs')", async () => {
    const { OrgHomeView } = await import("../apps/console/src/app/OrgHomeView");
    const html = renderToStaticMarkup(
      <OrgHomeView
        isLoading={true}
  ...
  // OrgSwitcherView ERROR renders a distinct error indicator, NOT "no orgs".
  it("OrgSwitcherView ERROR shows a distinct error indicator, never 'no orgs'", async () => {
  ...
  // OrgSwitcherView EMPTY still reads "no orgs" (unchanged), proving error ≠ empty.
  it("OrgSwitcherView EMPTY still reads 'no orgs' (unchanged)", async () => {
    const { OrgSwitcherView } = await import(
      "../apps/console/src/app/OrgSwitcherView"
    );
    const html = renderToStaticMarkup(
  ...
  // Rule-based, over EVERY file in apps/console/src rather than the old fixed file list: (1) only lib/supabase.ts may import a runtime value from @supabase/supabase-js (type-only
  ...
  it("only lib/supabase.ts value-imports @supabase/supabase-js, and nothing calls getSupabase() at module scope", async () => {
  ...
  // The two rules actually bite: known-bad snippets are flagged, type-only ones are not
  // (so the scan above can't pass vacuously).
  it("the rule checks flag value imports and module-scope getSupabase() calls, not type-only use", () => {
...
// Rule 1 — does this source import a RUNTIME value from @supabase/supabase-js? Static
...
// Rule 2 — is getSupabase() called at module scope: a column-0 call, or a top-level

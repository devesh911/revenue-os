...
# worker-webhook — thin router (skills advise, docs rule — CLAUDE.md)
...
3. `CLAUDE.md` Gotchas — webhooks arrive out of order: upsert by `provider_ref` (partial UNIQUE index, migration 012), order by `messages.seq`; pgboss schema has no RLS (by design); the `workflow_runs (status, wake_at)` scheduler index is sacred.

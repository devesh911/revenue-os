...
**Roles:** **Orchestrator (Fable, main thread)** = planner + assigner + reviewer only. **MODEL ROUTING v2 (2026-07-13, Devesh; STATE.md DECISIONS via PR #45):** ALL RED — including security/RLS/migration/guard-critical tasks — is authored by the **tester** agent (Opus 4.8, effort max); for that critical-path RED, the orchestrator reviews the returned tests **line-by-line** before any GREEN dispatch (test-is-spec review replaces self-authorship, §5). **Worker** (Opus 4.8, effort max) implements one task exactly. **Scout** (Haiku, read-only) recons docs/code to keep worker context lean. **Scribe** (Sonnet, effort max) writes PR/doc prose only.

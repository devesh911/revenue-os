...
**Phase note (D36):** S13.1's approval requirement and S13.6's human-merge gate are **LIVE-phase
controls**. While `STATE.md` says `PHASE: SETUP`, the owner has deliberately set approvals to 0
and granted merge-on-green to agent sessions (accepted risk recorded in D36; SETUP-era secrets
are treated as burned and rotate at go-live). The technical controls in this file — RLS, secret
hygiene, webhook verification, CI-as-trust-anchor, append-only history — are phase-independent
and never relaxed. `docs/runbooks/go-live.md` is the checklist that restores full force.
...
- [ ] **S13.2** Agent PAT is fine-grained: `contents:write` + `pull_requests:write` on this repo only. No admin, no actions, no secrets, no workflows scope. Lives only on the orchestrator machine. **Mechanism (D35):** GitHub restricts fine-grained PATs to repos *owned by the token's resource owner* — a collaborator bot on this personal repo cannot mint one. The bot therefore uses a **classic PAT, `repo` scope only (no `workflow`)**, with least privilege enforced at the ACCOUNT level: the bot is a Write collaborator on exactly this repository. The capability envelope is not assumed but **rehearsed** by `orchestrator/scripts/preflight_check.sh` before unattended runs unlock: CAN branch/commit/open PRs · CANNOT modify `.github/workflows` (push rejected without `workflow` scope), approve its own PRs, merge (ruleset + no bypass), or reach settings/secrets/rulesets (not admin).

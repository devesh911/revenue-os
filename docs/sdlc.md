...
| Console sign-in front door — `bun run local` env wrapper, dev login seeded by `db:seed`, fixed dev ports + local auth redirect URLs, worker PORT + PUT preflight, public /login with safe return-to, session + membership gates, cache wiped on sign-out/user switch, typed API errors + timeouts, 401 → signed out, GET /orgs ordered by name, Playwright sign-in spec armed in CI | ✅ | (this PR) | Devesh ask 2026-09-25 after a 9-agent blocker sweep (100 findings, 0 refuted); decisions: invite-only, email+password now (phone + Google next), admin MFA; RED `3d964ce` line-reviewed before GREEN; 3-lens review → 14 findings fixed (2 security: dot-segment open redirect, `?host=` local-guard bypass); no new deps, no migration |

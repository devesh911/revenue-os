...
| www rebuilt demo-first — one "Book a demo" journey (in-page Cal.com booking dialog, preview mode when unconfigured), controllable sample call (play/pause + duration, following transcript, result before playback), labelled illustrative proof, three steps + follow-up branch, three buyer demonstrations, compact pilot + fees in words + secondary built-vs-planned plans, FAQ, closing invitation, Plausible funnel events | ✅ | (this PR) | Devesh 12-point brief 2026-09-23; chose Cal.com, Plausible, a synthetic placeholder call, no rates; +zod (already T11-pinned) for the Cal.com boundary; www 147/0 (incl. vite build), repo-wide `bun test` 721/0 with CI env, typecheck 0, lint 0, audit clean, rls 0; built via multi-agent workflows (build → per-area review → whole-page review → fixes → verification) |

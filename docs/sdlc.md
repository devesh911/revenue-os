...
| CI guard S4.1 — no public IPv4 address in any tracked file (follow-up to the VPS-address incident) | ✅ | (this PR) | `scripts/guards.sh` `git grep -P` over tracked files, reserved/documentation ranges allowed, file:line only; `scripts/guards.test.ts` 4/0, RED 0/4 against the old script; goes green on main once #103 lands |

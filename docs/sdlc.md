...
| Analytics "Trends" — 30-day gap-filled daily series API + console wiring (task-51) | ✅ | (this PR) | `funnelTrends` (`packages/db/src/screens.ts`) via `GET /orgs/:orgId/metrics/trends` (S5.1 org-scope, role-gated 403); series mirrors funnelMetrics' tile predicates, gap-filled `generate_series(0,29)`; console Analytics Trends section wired (`useTrendsQuery`, CSS bar tracks, no chart dep); env-free green local (typecheck/lint, `apps/console/test/` 118/0); `services/worker/test/metrics-trends-api.test.ts` (tenancy) CI-owned. @1bf1ccb (feat/task-51-analytics-trends). |
| Console AgentsPage on real data — `GET /orgs/:orgId/agents` + `listAgentsAndWorkflows` (task-50) | ✅ | (this PR) | branch `feat/task-50-agents-api`; validate→authorize→do, `memberRole`→403, lean `{agents, workflows}` payload (S5.8 — no system_prompt/voice_config/language_config/tools_allowed/definition on the wire); `AgentsPage` wired via `useAgentsQuery` + DataShell/Table, prior placeholder retired; env-free gates green — typecheck, lint (182 files), `bun test apps/console/test/` 116/0 (after a tester mock-spread fix, PR #71 regression class); `agents-api.test.ts` (401/403/cross-tenant-denial/lean-shape/ordering) CI-owned; @d2be409 + @b46c47a |
| Guardrail-policies API + Settings Guardrails live on real data (task-52) | ✅ | (this PR) | GREEN @4c23611 (feat/task-52-guardrail-policies); one shared `GuardrailPolicyInputSchema` (packages/shared) parsed by both the worker PUT and the console mutation — the safety boundary for the FAIL-OPEN quiet-hours config read; `GET`/`PUT /orgs/:orgId/guardrail-policies` (member/admin S1.7); typecheck 0, lint clean (183 files), `guardrail-config.test.ts` 22/0, `apps/console/test/` 123/0; real-DB route suite (401/403 lattice incl. cross-tenant admin, round-trips, 400-nothing-written) CI-owned; no migration (guardrail_policies predates, 006_harness.sql) |
| www editorial redesign — Anthropic-inspired paper/ink/clay system, animated demo call + lead flow + moat art, pause switch (WCAG 2.2.2), mobile menu, deep links, favicon/meta | ✅ | #98 | Devesh ask 2026-09-23; no new deps; tests re-pinned to new tokens + sentence-case copy, + a11y/motion/separation pins; www 117/0, typecheck 0, biome 0, rls 0; worker suites need a worktree `.env` (CI-owned) |
| www demo-first copy inside the editorial design — one "Book a demo" journey (in-page Cal.com booking dialog, preview mode when unconfigured), sample recording in the hero, honest looping call, illustrative pilot report, three steps + follow-up in the engine panel, buyer demonstrations on the illustrated cards, pilot + fees in words + built-vs-planned plans, FAQ, closing invitation, Plausible funnel events | ✅ | (this PR) | Devesh 12-point brief 2026-09-23; built first as a restyle, which he rejected 2026-09-24 ("keep the copy changes, revert to the previous design") — rebuilt on the #98 design via multi-agent workflows (restore → rewire sections → tests → side-by-side review → fixes); chose Cal.com, Plausible, a synthetic placeholder call, no rates; +zod (already T11-pinned) for the Cal.com boundary; www 208/0 (incl. vite build), repo-wide `bun test` 782/0 with CI env, typecheck 0, lint 0, audit clean, rls 0 |
| www page as one enquiry in order + hero = board study A3 — from Devesh's picks on the hook-studies board (A3, B3, B5, C4) and his follow-up ("A3 as it is, remove the older call card … systematic information as they scroll"): How it works in chapters (01 the brief with Priya's reply + chime, 02 the Intent row opened up, 03 the call from 7:14 PM), one clock, one follow-up plan, pilot report after Pilot | ✅ | (this PR) | board (claude.ai artifact) → picks read from its db → built via multi-agent workflows (build → tests → review → fixes → two first-time-visitor flow reviews); lib/sequence, lib/chime; www 309/0, typecheck 0, lint 0, audit clean; DB-backed suites CI-verified |
| www componentization — React+Vite+Tailwind v4, `src/design`+`src/sections`+`src/content` separation (task-34) | ✅ | (this PR) | GREEN @f91d01d (feat/task-34-www-components); console's exact stack — react/vite/tailwindcss, already-approved [tech-stack §T24](tech-stack.md) pins, zero new BOM row; retires the week-3 Astro reservation; 6 design primitives + 9 sections + 5 typed content modules, `@theme` owns tokens/fonts, Pricing/Faq `useState` keep the `data-*` hooks; old `styles/*.css` deleted; RED @30b7cc1 retired the 68-pin static suite (each new spec traces its coverage to the retired AC-IDs, Fable line-reviewed all 3 files pre-GREEN); `apps/www/test/` 95/0, `apps/console/test/` 139/0, `tests/` 49/0, typecheck+lint clean; pixel parity pending a browser check |
| Memory-write M1 — end-of-call summaries fold into contact_memories (task-53) | ✅ | #88 | GREEN @f48dd68; same-transaction fold in the end-of-call-report branch — one kind='summary' row per report (verbatim, embedding NULL), previous live summary superseded PER CONTACT (bystander-pinned); silent skips (null contact / missing / empty summary); insert+supersede one writable CTE; no migration; 8/0 suite, full 551/0, typecheck 0, lint clean, rls 0 |
...
| Demo driver (`bun run demo`) + booking-path disposition defect fix (task-60) | ✅ | #94 | `bun run demo` drives the full engine cycle locally (no telephony, no LLM cost): `startRun` enroll → direct `scheduler.tick` → `place_call` through the real guard pipeline (stubbed voice sender) → a SIGNED end-of-call webhook through the REAL Vapi receiver → tick routes the disposition → terminal, printing the ordered journey; `bun run db:reset` now runs `scripts/db-reset.ts` (`supabase db reset` then the D31 app_service login restore, refuses non-local URLs). 🔴 REAL ENGINE DEFECT the RED suite caught and this PR fixes: the engine conflated a call's TECHNICAL outcome with its BUSINESS disposition in one `state.lastDisposition` slot, so the seeded `qualify_call → route(branch) → book` path could never reach `book` (every seeded run fell through `onDisposition['*']` → end; booking unreachable). Fix: `packages/harness/src/workflow/interpret.ts` routes an unnamed business disposition via the `completed` arm on call re-entry (`CALL_OUTCOMES` set gates the fallback); `services/worker/src/scheduler.ts` `drive()` preserves `lastDisposition` across a call→branch hop only (deletes it when `nextKind != branch`; call→call still clears, M2 P0 semantics preserved). Migration 017 adds `on delete cascade` to `webhook_events.org_id` (007_ops.sql), matching every other org-scoped harness table in 006 — org deletion previously 23503'd; expand-style, no data change. Gates: task suites 15/15, repo-wide `bun test` 647/0 (main baseline 632 + 15 new), typecheck 0, biome clean; live CLI run against the seeded real_estate org printed the full journey (enrolled → tick_advanced → call_placed → webhook_simulated (receiver 202) → run_advanced → terminal completed at 'end') and wrote qualified + booking outcome rows; RED authored with an explicit DO-NOT-WEAKEN defect note, zero test edits during GREEN. |
| Eval runner — `bun run evals` scenario-driven agent-quality gate (task-59, P4 activation-gate infra) | ✅ | #TBD | tester RED `scripts/{eval-runner,eval-assertions,eval-capture}.test.ts` + `tests/eval-runs-rls.test.ts` (Fable line-reviewed) → worker GREEN — drives a REAL conversation per [`eval_scenarios`](db-design.md) row through harness `runTurn` (real tool catalog; sends captured by an eval-mode port, never delivered); contact turns replayed by a v1 persona player, agent turns by the injected LLM provider (runtime = real Anthropic via `ANTHROPIC_API_KEY`, mocked in tests — Anthropic never called in tests); assertions are MECHANICAL v1 — `must_capture` read BACK from the contact row (ground truth, arg-echo impossible by design), `expect_outcome` matched against the append-only outcomes row; one [`eval_runs`](db-design.md) row per scenario (`passed`, per-assertion `scores` jsonb, `transcript_ref`); malformed scenario jsonb fails only that scenario, never the run; module split `scripts/eval-runner.ts` (testable lib) / `scripts/evals.ts` (thin CLI); new `ScenarioPersonaSchema`/`ScenarioScriptSchema`/`ScenarioAssertionsSchema` (`packages/shared/src/schemas.ts`, [dev-workflow §R6](dev-workflow.md)); `tsconfig.base.json` now excludes `**/*.test.ts` (restores the test-files-outside-typecheck-scope invariant, `scripts/` was the sole offending include glob); root devDeps `@revenue-os/{db,harness,shared}` added so `scripts/` can value-import them; gates: task suites 22/22, repo-wide 654/0 (main baseline 632 + 22 new), typecheck 0, biome 0, rls 0 offenders; disclosed + orchestrator-approved deviation — GREEN ran biome over the RED spec files (formatting + 2 dead imports, assert-preserving: `expect()` count 56/56 before/after) |
| VPS address out of the public repo + Cloudflare origin cert + `/ready` bearer token (domain due diligence) | ✅ | (this PR) | `scripts/provision-staging.sh` reads `VPS_HOST` (fails fast when unset); STATE.md address mentions replaced; Caddy `{$API_HOST}` + Origin CA `tls`, compose 443-only + `./certs` ro + `:?` blank-value refusal (a blank EDGE_SHARED_SECRET admitted an empty header — proven live); `/ready` behind `READY_TOKEN` (repro red on main: edge header alone read it); runbook §0 step zero + §2–§6; the burned IP itself is Devesh's (STATE WAITING) |
...
- **Gates:** domain purchase + a fresh VPS IP (Devesh) + `STAGING_SSH_KEY` env secret (command ready in STATE WAITING); the health gate sends `Authorization: Bearer $READY_TOKEN` to /ready.
...
- Cloudflare zone (api DNS, Transform Rule w/ EDGE_SHARED_SECRET, origin lockdown) · Pages custom
  domain · origin certificate + API_HOST · `docker compose --env-file ../.env up` · smoke. All steps scripted in

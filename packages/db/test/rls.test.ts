...
beforeAll(async () => {
	...
	await admin.query(`alter role app_service with login password 'app_service_local'`);
...
afterAll(async () => {
	await admin.query(`delete from orgs where slug in ('rls-test-a','rls-test-b')`);
...
describe("app_service client tenant isolation", () => {
	...
	it("withOrg(A) cannot read org B rows even when asked explicitly", async () => {
		const rows = await withOrg(appService, orgA, async (tx) => {
			const r = await tx.query(`select * from contacts where org_id = $1`, [orgB]);
			return r.rows;
		});
	...
	it("withOrg(A) cannot insert a contact into org B (RLS with-check)", async () => {
		expect(
			withOrg(appService, orgA, async (tx) => {
				await tx.query(`insert into contacts (org_id, first_name) values ($1, 'Mallory')`, [
					orgB,
				]);
			}),
